...
function success(c: any, data?: any, message = '') {
  return c.json(
    {
      success: true,
      message,
...
function failure(c: any, status: number, message: string) {
  return c.json(
    {
      success: false,
      message
...
function normalizePage(value: string | undefined | null, fallback: number): number {
...
function normalizePageSize(value: string | undefined | null, fallback: number): number {
  const parsed = Number(value);
  if (Number.isNaN(parsed) || parsed <= 0) {
    return fallback;
  }
...
function normalizeStatusFilter(value: string | undefined | null): string | null {
  if (!value) {
    return null;
  }
...
async function fetchModels(baseUrl: string, apiKey: string) {
  ...
  const models = normalizeModelsInput(data?.data ?? data);
...
async function loadChannels(c: any, where: string[], bindings: Array<string | number>) {
...
newapi.get('/', async (c) => {
  ...
  const pageSize = normalizePageSize(c.req.query('page_size') ?? c.req.query('limit'), 20);
  ...
  const whereSql = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
  const orderSql = idSort ? 'ORDER BY id ASC' : 'ORDER BY priority DESC, created_at DESC';
  ...
  const typeCounts: Record<string, number> = { all: Number(totalRow?.count ?? 0) };
  for (const entry of counts.results ?? []) {
    typeCounts[String(entry.type)] = Number(entry.count ?? 0);
  }

  const items = (rows.results ?? []).map((row) => {
    const channel = toNewApiChannel(row as any);
    delete (channel as any).key;
    return channel;
  });
...
newapi.get('/search', async (c) => {
  ...
  const filtered = rows.filter((row) => {
    const channel = row as any;
    const models = extractModelIds(channel as any);
...
newapi.get('/models', async (c) => {
  const result = await c.env.DB.prepare('SELECT * FROM channels WHERE status = ?')
    .bind('active')
    .all();
  const models = new Set<string>();
  for (const row of result.results ?? []) {
    for (const id of extractModelIds(row as any)) {
      models.add(id);
    }
  }
...
newapi.post('/', async (c) => {
  ...
  const exists = await c.env.DB.prepare('SELECT id FROM channels WHERE id = ?').bind(existingId).first();
  if (exists) {
...
newapi.put('/', async (c) => {
  ...
  const current = await c.env.DB.prepare('SELECT * FROM channels WHERE id = ?').bind(String(id)).first();
  if (!current) {
    return failure(c, 404, '渠道不存在');
  }

  const parsed = normalizeChannelInput(payload);
  const models = parsed.models.length > 0 ? parsed.models : extractModelIds(current as any);
  const mergedMetadata = mergeMetadata(current.metadata_json, parsed.metadata_json);
...
newapi.get('/test/:id', async (c) => {
  ...
  const result = await fetchModels(String(channel.base_url), String(channel.api_key));
  if (!result.ok) {
...
newapi.post('/test', async (c) => {
  ...
  const channel = await c.env.DB.prepare('SELECT * FROM channels WHERE id = ?').bind(String(id)).first();
  if (!channel) {
    return failure(c, 404, '渠道不存在');
  }
  const result = await fetchModels(String(channel.base_url), String(channel.api_key));
...
newapi.get('/:id', async (c) => {
  ...
  const output = toNewApiChannel(channel as any);
  return success(c, {
    ...output,
    model_mapping: metadata.model_mapping ?? '{}',
    channel_info: metadata.channel_info ?? { is_multi_key: false, multi_key_mode: 'random' }
  });
});
