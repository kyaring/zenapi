 ...
 */
function resolveChannelId(body: any): string | null {
...
channels.get('/', async (c) => {
  const result = await c.env.DB.prepare('SELECT * FROM channels ORDER BY created_at DESC').all();
...
channels.post('/', async (c) => {
  ...
  if (requestedId) {
    const exists = await c.env.DB.prepare('SELECT id FROM channels WHERE id = ?').bind(requestedId).first();
    if (exists) {
...
channels.patch('/:id', async (c) => {
  ...
  const current = await c.env.DB.prepare('SELECT * FROM channels WHERE id = ?').bind(id).first();
  if (!current) {
  ...
  await c.env.DB.prepare(
    ...
    .bind(
      ...
      normalizeBaseUrl(body.base_url ?? current.base_url),
...
channels.post('/:id/test', async (c) => {
  ...
  if (!response.ok) {
    await c.env.DB.prepare('UPDATE channels SET status = ?, updated_at = ? WHERE id = ?')
  ...
  await c.env.DB.prepare('UPDATE channels SET status = ?, models_json = ?, updated_at = ? WHERE id = ?')
    .bind('active', JSON.stringify(data), nowIso(), id)
    .run();

  return c.json({ ok: true, models: data?.data ?? [] });
});
