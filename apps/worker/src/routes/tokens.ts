...
tokens.post('/', async (c) => {
  ...
  const quotaTotal = body.quota_total === null || body.quota_total === undefined
    ? null
...
tokens.patch('/:id', async (c) => {
  ...
  const quotaTotalUpdate = body.quota_total === null || body.quota_total === undefined
    ? existing.quota_total
    : Number(body.quota_total);
  const quotaUsedUpdate = body.quota_used === null || body.quota_used === undefined
    ? existing.quota_used
...
tokens.get('/:id/reveal', async (c) => {
  const id = c.req.param('id');
  const record = await c.env.DB.prepare('SELECT token_plain FROM tokens WHERE id = ?').bind(id).first();
  if (!record) {
    return jsonError(c, 404, 'token_not_found', 'token_not_found');
  }
